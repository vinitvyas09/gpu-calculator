...
- **Rule of thumb**: `6ΨD` is accurate when the omitted quadratic attention term is small relative to model FLOPs: `12 × L × d_attn × s / (6Ψ)`. For dense transformers with `Ψ ≈ 12Ld²` and `d_attn ≈ d`, this fraction is approximately `s / (6d)`. A 10% materiality threshold therefore requires roughly `d >= 1.7s`, not merely `d > s/12`.
- When the overhead fraction exceeds the materiality threshold, the quadratic attention term becomes significant: e.g., 175B-class dense models at s=2048 are under ~3%, at s=4096 around ~5%, and at s=32768 roughly 30-45% depending on the exact parameter count and attention projection width. Models under ~13B can exceed 30% at moderate context lengths.
...
- The calculator should always use the PaLM formula and display the attention overhead percentage so users understand the cost of long sequences. It should flag when the attention overhead fraction shows the simplified `6ΨD` formula would omit material work.
...
**Calculator use**: Given the user's chosen token batch size `B_tok = b × s × G × N_dp` and the predicted training loss from Section 4.3, the calculator should display: (1) whether `B_tok` is above or below `B_crit`, (2) the compute multiplier above optimum, and (3) the wasted-compute fraction. For example: "Your token batch of 4M is 2x B_crit -- you are using 3x the theoretical minimum compute, and 67% of the compute in this run is overhead relative to the optimum." If `D < B_tok`, the modeled run has only one partial optimizer step; use `D` as the actual batch-efficiency token batch unless the user explicitly assumes padding, dropping, or reusing examples. This is advisory only -- many practical constraints (memory, hardware utilization, training stability) override the theoretical optimum.
...
**Context parallelism** (Meta, Llama 3 scaling): Context parallelism (CP) shards the input sequence along the sequence dimension across N_cp GPUs. Each CP rank processes `s/N_cp` query tokens while attending over the full sequence via exchanged KV tensors. In activation memory formulas above, replace the linear sequence dimension with `s/N_cp` when CP is active. For non-Flash attention, the materialized score matrix is local-query by global-key, so the quadratic term scales as `s^2 / N_cp`, not `s^2 / N_cp^2`:
```
M_act_layer = (s/N_cp) × b × d × (34 + 5 × a × s / d) bytes  [no checkpointing, with CP]
```
With Flash Attention the `5as/d` term disappears as usual. CP communication cost is an all-gather of KV tensors per layer (forward) and reduce-scatter of KV gradients (backward). Because communication is O(s) while attention compute is O(s^2), CP overhead shrinks with longer sequences, making it most effective at 32K+ sequence lengths. When to use CP: when sequence length causes activation memory pressure and the micro-batch size would otherwise drop to 1. The trigger heuristic is `B_seq / (N_gpu / (N_tp × N_pp)) <= 1` at long sequence lengths, indicating DP alone cannot maintain throughput. CP should only be enabled when `s/N_cp` still exceeds a minimum chunk size (~2K tokens) to maintain sufficient arithmetic intensity per rank.
...
- **EP routing all-to-all staging**: dispatch/combine buffers use the local token shard. With context parallelism and sequence parallelism this is `s / (N_cp × N_sp)`, where `N_sp = N_tp` when sequence parallelism is enabled and `1` otherwise.
...
**TF32 (TensorFloat-32) note**: TF32 is a **compute mode**, not a storage format — it uses 19-bit precision internally in matrix/tensor cores (10-bit mantissa of FP16, 8-bit exponent of FP32) but all tensors remain stored as FP32 (4 bytes/element) in memory. TF32-capable training stacks often enable TF32 matmul for throughput, but vanilla PyTorch CUDA matmul defaults to strict FP32 in PyTorch 1.12+ unless `torch.set_float32_matmul_precision("high" | "medium")` or `torch.backends.cuda.matmul.allow_tf32 = True` is set. AMD CDNA 3 accelerators such as MI300X also expose TF32 matrix throughput; MI200-series accelerators do not. When the user selects "FP32 training" on hardware with TF32 support, the calculator should use the TF32 TFLOPS rate (not the non-tensor-core/vector FP32 rate) as an explicit TF32-enabled estimate. Without this adjustment, FP32 training time estimates for TF32-enabled runs would be several times too pessimistic. TF32 does not affect memory calculations — all tensors remain in FP32 at 4 bytes per element. Pre-Ampere GPUs (V100, T4) and AMD MI200-series GPUs should use the non-tensor-core/vector FP32 rate (e.g., V100: 15.7 TFLOPS). For consumer GPUs (RTX 30xx/40xx), TF32 TFLOPS listed are tensor core rates.
...
**QLoRA loading memory**: HuggingFace/bitsandbytes loads models layer-by-layer: each layer is loaded in fp16 on CPU, quantized to NF4, then the NF4 version is moved to GPU. The GPU never holds the full fp16 model -- its peak during loading is approximately `M_nf4_total + max_parameter_unit_size × 2` (the accumulated NF4 model plus one transformer block or embedding/output-head unit in fp16 at a time), which is only slightly above steady-state. However, CPU memory requires ~2Ψ bytes to hold the full fp16 model during this process. The calculator should warn about the CPU memory requirement (~2Ψ) when it exceeds available system RAM.
...
**Partial final batches**: Post-training time and memory should use the largest prompt batch that is actually modeled. If `dataset examples x epochs` produces at least one full configured batch, size memory for the configured batch. If the run has only a final partial batch, size DPO chosen/rejected activations, PPO/GRPO rollout buffers, KV cache, generation capacity, and GPU-fit estimates from that partial batch (rounded up to whole examples for memory), not from the unused configured maximum.
...
- **Modern open-weight heuristic** (`Psi >= 2B`): `d_ff = round_to_128(8/3 * d)`, `a_kv = largest divisor of a that is <= 8`, `V = 128,000`
